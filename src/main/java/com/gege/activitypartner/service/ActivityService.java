...
public class ActivityService {
  ...
  public ActivityResponseDTO createActivity(ActivityRequestDTO request, Long creatorId) {
    User creator =
        userRepository
            .findById(creatorId)
            .orElseThrow(
                () -> new ResourceNotFoundException("User not found with id: " + creatorId));
    ...
    activity.setLatitude(
        request.getLatitude() != null ? BigDecimal.valueOf(request.getLatitude()) : null);
    activity.setLongitude(
        request.getLongitude() != null ? BigDecimal.valueOf(request.getLongitude()) : null);
  ...
  @Transactional(readOnly = true)
  public List<ActivityResponseDTO> getAllActivitiesWithDistance(
  ...
  public List<ActivityResponseDTO> getMyActivities(Long creatorId, ActivityStatus status) {
    ...
    if (status == null) {
      ...
      activities =
          activityRepository.findByCreatorId(creatorId).stream()
  ...
  public List<ActivityResponseDTO> getAvailableUpcomingActivities(Long userId) {
    ...
    List<ActivityResponseDTO> activityResponseDTOS =
        activityRepository.findAvailableUpcomingActivities(LocalDateTime.now()).stream()
            .map(this::mapToResponseDTO)
            .collect(Collectors.toList());
    ...
    for (ActivityResponseDTO activityResponseDTO : activityResponseDTOS) {
      ...
      if (currentUser.getLatitude() != null && currentUser.getLongitude() != null) {
        activityResponseDTO.setDistance(
            DistanceCalculator.calculateDistance(
                currentUser.getLatitude(),
                currentUser.getLongitude(),
                activityResponseDTO.getLatitude(),
                activityResponseDTO.getLongitude()));
      } else {
  ...
  public List<ActivityResponseDTO> getRecommendedActivities(Long userId) {
    ...
    return allActivities.stream()
        ...
        .filter(
            activity ->
                !activity.getCreator().getId().equals(userId)) // Exclude user's own activities
  ...
  private ActivityResponseDTO mapToResponseDTO(
      Activity activity, BigDecimal userLatitude, BigDecimal userLongitude) {
    ...
    if (userLatitude != null
      ...
      double distance =
          DistanceCalculator.calculateDistance(
              userLatitude, userLongitude, activity.getLatitude(), activity.getLongitude());
