...
public class ActivityParticipantService {
    ...
    if (existingParticipant.isPresent()) {
      ...
      if (status == ParticipantStatus.DECLINED) {
        throw new InvalidParticipantActionException(
            "Your application was declined. You cannot reapply to this activity.");
      }
  ...
  public List<ParticipantResponse> getInterestedUsers(Long activityId, Long creatorId) {
    ...
    if (!activity.getCreator().getId().equals(creatorId)) {
      throw new InvalidParticipantActionException(
          "Only the activity creator can view interested users");
    }
  ...
  @Transactional(readOnly = true)
  public List<ParticipantActivityResponse> getMyParticipationsByStatus(
      Long userId, ParticipantStatus status) {
    if (!userRepository.existsById(userId)) {
      throw new ResourceNotFoundException("User not found with id: " + userId);
    }
  ...
  public ParticipantResponse updateParticipantStatus(
      Long participantId, ParticipantStatus newStatus, Long creatorId) {
    ActivityParticipant participant =
        participantRepository
            .findByIdWithDetails(participantId)
            .orElseThrow(
                () ->
    ...
    if (!participant.getActivity().getCreator().getId().equals(creatorId)) {
      throw new InvalidParticipantActionException(
          "Only the activity creator can update participant status");
    }
  ...
  public ParticipantResponse confirmJoining(Long participantId, Long userId) {
    ...
    if (participant.getStatus() != ParticipantStatus.ACCEPTED) {
      throw new InvalidParticipantActionException(
          "Can only confirm participation after being accepted");
    }
  ...
  public void deleteInterest(Long activityId, Long userId) {
    ...
    if (participant.getStatus() != ParticipantStatus.WITHDRAWN) {
      throw new InvalidParticipantActionException(
          "Can only delete after withdrawing interest. Use leaveActivity to withdraw.");
    }
  ...
  private ParticipantActivityResponse mapToParticipantActivityResponse(
      ActivityParticipant participant) {
