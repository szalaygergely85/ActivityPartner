...
public class ReviewService {
  ...
  public ReviewResponse createReview(ReviewRequest request, Long reviewerId) {
    ...
    User reviewedUser =
        userRepository
            .findById(request.getReviewedUserId())
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        "Reviewed user not found with id: " + request.getReviewedUserId()));
  ...
  private void validateReviewCreation(User reviewer, User reviewedUser, Activity activity) {
    ...
    if (!activityEnded) {
      throw new InvalidParticipantActionException("Can only review after the activity has ended");
    }
  }
