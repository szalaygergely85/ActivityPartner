...
public class UserPhotoService {
  ...
  public String uploadUserPhoto(Long userId, MultipartFile file) {
    ...
    if (photoCount >= 6) {
      throw new InvalidParticipantActionException(
          "Maximum 6 photos allowed per user. Please delete one before uploading a new one.");
    }
  ...
  public UserPhotoResponse setAsProfilePicture(Long userId, Long photoId) {
    ...
    userPhotoRepository
        .findByUserIdAndIsProfilePictureTrue(userId)
        .ifPresent(
            currentProfile -> {
  ...
  public void deleteUserPhoto(Long userId, Long photoId) {
    ...
    if (photo.getIsProfilePicture()) {
      List<UserPhoto> remainingPhotos =
          userPhotoRepository.findByUserIdOrderByDisplayOrder(userId).stream()
