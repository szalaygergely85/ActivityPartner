...
public interface ReportRepository extends JpaRepository<Report, Long> {
  ...
  @Query(
      "SELECT r FROM Report r WHERE r.reportedActivity.id = :activityId AND r.reporter.id = :reporterId")
  List<Report> findByActivityIdAndReporterId(
      @Param("activityId") Long activityId, @Param("reporterId") Long reporterId);

  @Query(
      "SELECT r FROM Report r WHERE r.reportedMessage.id = :messageId AND r.reporter.id = :reporterId")
  List<Report> findByMessageIdAndReporterId(
      @Param("messageId") Long messageId, @Param("reporterId") Long reporterId);
}
