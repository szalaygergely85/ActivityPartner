...
public class ReportController {
  ...
  /** Submit a new report POST /api/reports */
  @PostMapping
  ...
  /** Get all reports submitted by current user GET /api/reports/my-reports */
  ...
  /** Get all pending reports (admin only) GET /api/reports/pending */
  @GetMapping("/pending")
  ...
  /** Get reports by type and status (admin only) GET /api/reports?type=ACTIVITY&status=PENDING */
  @GetMapping
  @PreAuthorize("hasRole('ADMIN')")
  public ResponseEntity<List<ReportResponse>> getReportsByTypeAndStatus(
      @RequestParam ReportType type, @RequestParam ReportStatus status) {

    List<ReportResponse> reports = reportService.getReportsByTypeAndStatus(type, status);
    return ResponseEntity.ok(reports);
  }

  /** Update report status (admin only) PATCH /api/reports/{reportId}/status */
  ...
  /** Get report count for an activity GET /api/reports/activity/{activityId}/count */
  ...
  /** Get report count for a message GET /api/reports/message/{messageId}/count */
